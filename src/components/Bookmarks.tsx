...
import { Bookmark, Building2, Calendar, ChevronDown, Code2, Loader2, MessageSquare, User } from "lucide-react";
...
function Bookmarks() {
    ...
    const safeBookedQuestions = Array.isArray(bookedQuestions) ? bookedQuestions : [];
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-2 gap-6">
                    {isControllerLoading && safeBookedQuestions.length === 0 ? (
                        ...
                        safeBookedQuestions.map((bookmark) => (
                            <div 
                                key={bookmark._id}
                                onClick={() => router.push(`/pages/Questions/${bookmark?.questionId?._id}`)}
                                    ...
                                    <div className="flex items-start justify-between mb-4">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-sm text-slate-500">{bookmark.questionId?.role || 'No role specified'}</p>
                                            </div>
                                        </div>
                                    ...
                                    {bookmark.questionId?.solution && (
                                        <div className="mb-4 p-3 bg-green-50 border border-green-200 rounded-lg">
                                            <div className="flex items-center gap-2">
                                                ...
                                                <span className="text-sm font-medium text-green-800">Solution Included</span>
                                            </div>
                                        </div>
                                    )}
                                        ...
                                        <div className="flex items-center gap-2">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-xs text-slate-500 flex items-center gap-1">
                                                    ...
                                                    {bookmark.questionId?.createdAt ? formatDate(bookmark.questionId.createdAt) : 'Unknown date'}
                                                </p>
                                            </div>
                                        </div>
