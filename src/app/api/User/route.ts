...
export const DELETE = async() =>{
    ...
    try {
        ...
        if(!profile || !questions) return NextResponse.json({ message: "not found" },{ status: 404 });
...
export const PUT = async(req: NextRequest) =>{
    ...
    try {
        ...
        const profile = await userModel.findById({ userId },{ username: body.username },{ new: true });
